...
from projects.custom_objs import BigCubeObject, BatteryWithHandlesObject
...
from projects.custom_objs import NarrowGapArena
...
class ExpNarrowGap(ManipulationEnv):
    def __init__(
        self,
        robots,
        ...
        log_data=False, # for creating HDF5 log files
        log_filename=None, 
    ):
        ...
        if log_filename is None:
            ...
            self.log_filename = f"explog_narrowgap{current_time}.h5"
    ...
    def _load_model(self):
        ...
        self.pot = BatteryWithHandlesObject(name="pot")
        self.visual_pot = PotVisualObject(name="vis_pot")

        self.objs = [self.pot]
        self.objs_cls = [BatteryWithHandlesObject]
        self.objs_xpos = [[-0.08, -0.08]]
        self.objs_ypos = [[0, 0]]
        self.objs_rots = [[np.pi/2, np.pi/2]]
        self.objs_zpos = [0.15]

        # for goal checking
        # shelf_top_pos = self.mujoco_arena.shelf_top_pos
        # self.objs_xpos = [[shelf_top_pos[0], shelf_top_pos[0]]]
        # self.objs_ypos = [[shelf_top_pos[1], shelf_top_pos[1]]]
        # self.objs_rots = [[0, 0]]
        # self.objs_zpos = [shelf_top_pos[2] + 0.07] # shelf height + half visual pot height
    ...
    def _setup_references(self):
        ...
        for geom in self.sim.model.geom_names:
            if "shelf" in geom or "table" in geom or "block" in geom:
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            # Position visual pot above shelf (fixed position)
            shelf_top_pos = self.mujoco_arena.shelf_top_pos
            visual_pot_body_id = self.sim.model.body_name2id(self.visual_pot.root_body)
            self.sim.model.body_pos[visual_pot_body_id] = shelf_top_pos + [0, 0, 0.07]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Narrow Gap Environment with XR or MediaPipe Teleoperation")
