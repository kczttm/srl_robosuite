...
from robosuite.models.objects import PotWithHandlesObject #, BottleObject
# from projects.custom_objs import BigCubeObject
...
from projects.custom_objs import CabinetArena
from projects.custom_objs import PotVisualObject
...
class ExpCabinet(ManipulationEnv):
    def __init__(
        self,
        robots,
        ...
        self.mujoco_arena = None  # for logging shelf position
        ...
        if log_filename is None:
            ...
            self.log_filename = f"explog_cabinet{current_time}.h5"
    ...
    def _load_model(self):
        ...
        self.mujoco_arena = mujoco_arena  # store for later

        # self.cube1 = BigCubeObject(name="cube1")
        # self.cube2 = BigCubeObject(name="cube2")

        self.pot = PotWithHandlesObject(name="pot")
        self.visual_pot = PotVisualObject(name="vis_pot")

        self.objs = [self.pot]
        self.objs_cls = [PotWithHandlesObject]
        self.objs_xpos = [[0, 0]]
        self.objs_ypos = [[0.0, 0.0]]
        self.objs_rots = [[np.pi/2, np.pi/2]]
        self.objs_zpos = [0.22]
        # for goal testing only
        # goal_pos = self.mujoco_arena.cabinet_bottom_pos - [0, 0, 0.3]
        # self.objs_xpos = [[goal_pos[0], goal_pos[0]]]
        # self.objs_ypos = [[goal_pos[1], goal_pos[1]]]
        # self.objs_rots = [[0, 0]]
        # self.objs_zpos = [goal_pos[2]]
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=self.objs + [self.visual_pot],
        )
    ...
    def _setup_references(self):
        ...
        # two handle pot visual
        self.visual_pot_body_id = self.sim.model.body_name2id(self.visual_pot.root_body)
        self.visual_handle0_site_id = self.sim.model.site_name2id(self.visual_pot.important_sites["handle0"])
        self.visual_handle1_site_id = self.sim.model.site_name2id(self.visual_pot.important_sites["handle1"])
        self.visual_pot_center_id = self.sim.model.site_name2id(self.visual_pot.important_sites["center"])

        # table
        self.table_top_id = self.sim.model.site_name2id("table_top")

        # cabinet
        self.cabinet_id = self.sim.model.body_name2id("cabinet")
        self.cabinet_door_id = self.sim.model.body_name2id("door")
        self.cabinet_handle_id = self.sim.model.body_name2id("latch")

        # for collision detection
        environment_geoms = ['table_collision', 'l_frame', 'r_frame', 'back_frame', 'bot_frame', 'top_frame']
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            # Position visual pot above cabinet (fixed position)
            cabinet_bottom_pos = self.mujoco_arena.cabinet_bottom_pos
            visual_pot_body_id = self.sim.model.body_name2id(self.visual_pot.root_body)
            self.sim.model.body_pos[visual_pot_body_id] = cabinet_bottom_pos + [0, 0, 0.07]
            # self.sim.model.body_quat[visual_pot_body_id] = [1, 0, 0, 0]  # No rotation
    ...
    def step(self, action):
        ...
        pot_pos = self.sim.data.get_body_xpos("pot_root")
        vis_pot_pos = self.sim.data.get_body_xpos("vis_pot_main")
        self.contact_logger.goal_check(pot_pos, vis_pot_pos)
        if self.log_data:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Cabinet Environment with XR or MediaPipe Teleoperation")
